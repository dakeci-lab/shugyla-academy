...
function DetailRow({ label, value }) {
  ...
  return (
    <div className="supplier-details__row">
      ...
      <span className="supplier-details__value">{display}</span>
    </div>
  )
}
    ...
    <div className="supplier-details">
      <div className="supplier-details__header">
        <div>
          ...
          <p className="supplier-details__legal">
            {supplier.legalName || '—'}
          </p>
        </div>
      ...
      <div className="supplier-details__grid">
        <section className="supplier-details__section">
          ...
          <DetailRow label="Юридическое название" value={supplier.legalName} />
          <DetailRow label="Категории" value={formatSupplierCategories(supplier.productCategories)} />
          <DetailRow label="Статус" value={SUPPLIER_STATUS_LABELS[supplier.status]} />
        </section>
        ...
        <section className="supplier-details__section">
          ...
          <DetailRow label="Ответственный сотрудник" value={supplier.responsibleEmployeeName} />
        </section>

        <section className="supplier-details__section">
          <h3>Логистика</h3>
          <DetailRow label="Дни приёма заказа" value={supplier.orderDays} />
        ...
        <section className="supplier-details__section">
          <h3>Условия</h3>
          <DetailRow label="Условия оплаты" value={PAYMENT_TYPE_LABELS[supplier.paymentType]} />
          <DetailRow
            label="Срок отсрочки"
            value={
              supplier.deferralDays != null && supplier.deferralDays !== ''
                ? `${supplier.deferralDays} дн.`
                : null
            }
          />
        ...
        <section className="supplier-details__section supplier-details__section--wide">
          ...
          <p className="supplier-details__comment">
            {supplier.comment || '—'}
          </p>
        </section>
      </div>
      ...
      {(canEdit || canArchive || canDelete) && (
        <div className="supplier-details__actions">
          ...
          {canDelete && (
            <button
              type="button"
              className="btn btn--ghost supplier-details__delete"
