...
function stageStatic() {
  ...
  assert('run_id pattern TT-ADMIN-ESC-E2E', shared.includes('TT-ADMIN-ESC-E2E'))
  assert('recipient override key', shared.includes('recipient_employee_ids'))
  assert('suppress employee push key', shared.includes('suppress_employee_push'))
  assert('escalation_only key', shared.includes('escalation_only'))
  ...
  assert('edge passes recipient override', edge.includes('controlledRecipientIds'))
  ...
  assert('scheduler escalations', scheduler.includes('dispatchAdminEscalations'))
