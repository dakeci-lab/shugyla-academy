...
function stageMetadata() {
  console.log('Stage 3: Filename and UMAG comment')

  check(
    'versioned filename exact',
    buildReceivingUmagFilename({
      purchaseOrderNumber: 'PO-1842',
      invoiceNumbers: ['INV-18275', '18276'],
      version: 2,
    }) === 'UMAG_PO-1842_INV-18275-18276_v2.xlsx'
  )
  check(
    'missing invoice has stable filename fallback',
    buildReceivingUmagFilename({ orderNumber: '77' }) ===
      'UMAG_PO-77_INV-NO-INVOICE_v1.xlsx'
  )
  const unsafe = buildReceivingUmagFilename({
    orderNumber: '../PO:42',
    invoiceNumber: 'INV/../../bad',
    version: -3,
  })
  check('filename removes path separators', !unsafe.includes('/') && !unsafe.includes('\\'))
  check('invalid version falls back to v1', unsafe.endsWith('_v1.xlsx'))
  check('filename sanitizer has explicit fallback', sanitizeReceivingUmagFilenamePart('...') === 'UNKNOWN')
  check(
