...
function stageStatic() {
  ...
  assert('connected status', ui.includes('Подключено'))
  assert('reconnection status', ui.includes('Требуется переподключение'))
  assert('not connected status', ui.includes('Подключите уведомления'))
  assert('install pwa status', ui.includes('Установите PWA'))
  assert('denied status with iphone hint', ui.includes('Найдите Shugyla Platform'))
  assert('unsupported status', ui.includes('Уведомления не поддерживаются на этом устройстве'))
  assert('error status', ui.includes('Ошибка подключения'))
  assert('success hint', ui.includes('CONNECT_SUCCESS_HINT'))
  assert('disconnect confirm', ui.includes('Вы перестанете получать напоминания'))
