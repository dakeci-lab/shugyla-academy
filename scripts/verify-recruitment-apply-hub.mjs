...
const publicListFunction = migration.slice(
...
assert('hub is the only list renderer', !read('src/pages/VacanciesPage.jsx').includes('fetchPublishedVacanciesForApply'))
assert(
...
assert(
  'migration whitelist only',
  publicListFunction.includes('returns table') && !publicListFunction.includes('select *')
)
...
assert(
  'getApplyHubUrl uses canonical careers origin',
  recruitmentData.includes('getApplyHubUrl') &&
    recruitmentData.includes('getCareersUrl') &&
    hostSurface.includes('VITE_CAREERS_ORIGIN')
)
assert('production careers origin is jobs', hostSurface.includes('https://jobs.shugyla-market.kz'))
