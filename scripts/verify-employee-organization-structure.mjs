...
function main() {
  ...
  assert('global summary counters removed', !orgList.includes('Показано:'))
  assert('group employee count label removed', !orgList.includes('сотрудник') || orgList.includes('aria-label'))
  assert('unified group table', orgList.includes('employee-org-table') && orgList.includes('colgroup'))
  assert('position separator row', orgList.includes('employee-org-table__position-row'))
  assert('edit action in org list', orgList.includes('Редактировать сотрудника'))
  assert('empty clear search action', orgList.includes('Очистить поиск'))
  assert('empty title', orgList.includes('Сотрудники не найдены'))
  ...
  assert('fixed table layout', orgCss.includes('table-layout: fixed'))
  assert('group green level', orgCss.includes('#e8f5ec') || orgCss.includes('#E8F5EC'))
  assert('position green level', orgCss.includes('#f3faf5') || orgCss.includes('#F3FAF5'))
  assert('employee hover green', orgCss.includes('#f7fcf8') || orgCss.includes('#F7FCF8'))
  assert('action column width fixed', orgCss.includes('employee-org-table__col-actions'))
