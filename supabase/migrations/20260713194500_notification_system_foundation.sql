-- Notification system foundation for Shugyla Platform
-- Idempotent where possible. Safe to re-run. Does not apply remote changes by itself.
--
-- Includes:
--   academy_users.auth_user_id + backfill
...
--   RLS policies, seed templates/rules (disabled)

-- Prevent concurrent execution
select pg_advisory_xact_lock(202607131945);

-- ---------------------------------------------------------------------------
-- 0. Helpers
-- ---------------------------------------------------------------------------

-- Mirrors src/utils/phoneUtils.js loginToTechnicalEmail() for safe backfill.
create or replace function notification_login_to_technical_email(p_login text)
returns text
language plpgsql
immutable
as $$
...
comment on function notification_login_to_technical_email(text) is
  'Computes Supabase Auth technical email from academy_users.login (mirrors loginToTechnicalEmail in phoneUtils.js).';

-- ---------------------------------------------------------------------------
-- 1. Link academy_users to Supabase Auth
-- ---------------------------------------------------------------------------
...
alter table public.academy_users
  add constraint academy_users_auth_user_id_unique
  unique (auth_user_id);
...
-- Backfill: link only on unambiguous email match. Never overwrite existing auth_user_id.
with candidates as (
  select
