...
- Убыточные SKU: отрицательная `profit_8w` сохраняется как есть. Класс при метрике ≤ 0 — `NULL` / в UI «—», никогда A/B/C (как у qty и revenue). Парето 80/95 считается только по положительной прибыли. Уже записанные C на убыточных строках остаются до следующего ручного sync после деплоя Edge.
