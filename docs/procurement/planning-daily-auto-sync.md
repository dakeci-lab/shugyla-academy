...
## 0. Статус: применено и подтверждено на проде (2026-09-02)

Секрет создан в vault, переменные окружения выставлены, `umag-procurement`
задеплоена, миграция применена — `cron.job` `jobid=3`
(`procurement-sync-scheduler-daily-0700-aqtobe`, `0 2 * * *`, `active=true`).
Ручной прогон `select public.invoke_procurement_sync_scheduler();` сразу
после применения создал реальный снимок:
`procurement_snapshots.id = 7834d7be-149e-4d74-a905-23194f6fa6af`,
`status = 'ready'`, `created_by = 'system'`,
`created_by_name = 'Автосинхронизация (07:00)'` — цепочка HMAC-подпись →
`umag-procurement` → `handleSync` подтверждена end-to-end на живых данных.
Первый штатный (по расписанию) запуск — 02:00 UTC / 07:00 Актобе.
